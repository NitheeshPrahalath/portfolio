...
# Computer Networking
    ...
    Computer network is a collection of interconnected devices that communicate with each other.

Nodes are physical devices, Routers and switches control the flow of information.

Transmission media carry data from one device to anotther.

Wired media - Ethernet and optical fiber cables

Wireless media- wi-fi, radio signals

A data sent from one device is broken into small packets and transmitted across the network.

Protocols define how data packets are formatted, transmitted,received and acknowleged.

Each device has unique IP address for correct destination.


# Types of Computer Network Architectu0re

## 1. Client- Server

    Represnts a type of architecture in which nodes functioon as servers or clients, where the server manages client behavior

## 2. Peer-Peer

    Operates without any central serveer, allowing each device to act as either a client or as a server.
